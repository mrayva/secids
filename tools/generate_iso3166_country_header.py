...
lines.append('')
lines.append('constexpr bool is_upper_alpha(char c) noexcept { return c >= \"A\"[0] && c <= \"Z\"[0]; }')
lines.append('constexpr bool is_digit(char c) noexcept { return c >= \"0\"[0] && c <= \"9\"[0]; }')
lines.append('constexpr char to_upper_ascii(char c) noexcept { return (c >= \"a\"[0] && c <= \"z\"[0]) ? static_cast<char>(c - \"a\"[0] + \"A\"[0]) : c; }')
lines.append('')
lines.append('} // namespace detail')
lines.append('')
lines.append('constexpr std::optional<alpha2_packed_type> pack_alpha2(std::string_view code) noexcept {')
lines.append('    if (code.size() != 2) return std::nullopt;')
lines.append('    alpha2_packed_type value = 0;')
lines.append('    for (char c : code) {')
lines.append('        c = detail::to_upper_ascii(c);')
lines.append('        if (!detail::is_upper_alpha(c)) return std::nullopt;')
lines.append('        value = static_cast<alpha2_packed_type>((value << 8U) | static_cast<unsigned char>(c));')
lines.append('    }')
...
lines.append('    if (code.size() != 2) return std::nullopt;')
lines.append('    row_index_type value = 0;')
lines.append('    for (char c : code) {')
lines.append('        c = detail::to_upper_ascii(c);')
lines.append('        if (!detail::is_upper_alpha(c)) return std::nullopt;')
lines.append('        value = static_cast<row_index_type>(value * 26U + static_cast<row_index_type>(c - \"A\"[0]));')
lines.append('    }')
lines.append('    return value;')
lines.append('}')
lines.append('')
lines.append('constexpr std::optional<alpha3_packed_type> pack_alpha3(std::string_view code) noexcept {')
lines.append('    if (code.size() != 3) return std::nullopt;')
lines.append('    alpha3_packed_type value = 0;')
lines.append('    for (char c : code) {')
lines.append('        c = detail::to_upper_ascii(c);')
lines.append('        if (!detail::is_upper_alpha(c)) return std::nullopt;')
lines.append('        value = static_cast<alpha3_packed_type>(value * 26U + static_cast<alpha3_packed_type>(c - \"A\"[0]));')
lines.append('    }')
lines.append('    return value;')
lines.append('}')
lines.append('')
lines.append('inline std::string unpack_alpha2(alpha2_packed_type value) {')
lines.append('    std::string out(2, \"\\0\"[0]);')
lines.append('    out[0] = static_cast<char>((value >> 8U) & 0xFFU);')
lines.append('    out[1] = static_cast<char>(value & 0xFFU);')
lines.append('    return out;')
lines.append('}')
lines.append('')
lines.append('inline std::string unpack_alpha3(alpha3_packed_type value) {')
lines.append('    std::string out(3, \"\\0\"[0]);')
lines.append('    out[2] = static_cast<char>(\"A\"[0] + (value % 26U));')
lines.append('    value = static_cast<alpha3_packed_type>(value / 26U);')
lines.append('    out[1] = static_cast<char>(\"A\"[0] + (value % 26U));')
lines.append('    value = static_cast<alpha3_packed_type>(value / 26U);')
lines.append('    out[0] = static_cast<char>(\"A\"[0] + (value % 26U));')
lines.append('    return out;')
lines.append('}')
lines.append('')
...
for row in rows:
    ...
    lines.append(f'        static_cast<alpha2_packed_type>(0x{alpha2:04X}u),')
