...
int main() {
    ...
    assert(vocab_snapshot::available());
    ...
    write_file(tmp / "raw" / "micCode.json", "{\"values\":[\"XNYS\", \"XNAS\", \"XNYS\", \"ARCX\"]}\n");

    auto snapshot = build_snapshot_from_raw_directory(tmp / "raw", vocab_domain::mic_code);
    assert(snapshot.size() == 3);
    assert(snapshot.find_id("ARCX") == 0);
    assert(snapshot.find_id("XNAS") == 1);
    assert(snapshot.find_id("XNYS") == 2);
    ...
    std::ifstream manifest(tmp / "snapshots" / "manifest.json");
    const std::string manifest_text{
        std::istreambuf_iterator<char>(manifest), std::istreambuf_iterator<char>()};
    assert(manifest_text.find("\"version\": 2") != std::string::npos);
