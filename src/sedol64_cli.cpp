...
std::optional<secids::sedol64::value_type> parse_u64(std::string_view text) {
    if (text.empty()) {
        return std::nullopt;
    }

    secids::sedol64::value_type value = 0;
    for (char c : text) {
        if (c < '0' || c > '9') {
            return std::nullopt;
        }
        const auto digit = static_cast<secids::sedol64::value_type>(c - '0');
